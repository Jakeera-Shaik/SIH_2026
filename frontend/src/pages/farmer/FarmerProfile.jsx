...
import { User, Sprout, MapPin, Phone, Mail, ShieldCheck, CheckCircle2 } from 'lucide-react';
...
export const FarmerProfile = () => {
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div>
        <h1 className="text-2xl font-black text-slate-100">Farmer Account Profile</h1>
        <p className="text-xs text-slate-400 mt-1">Manage your location settings and farm details.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-xl space-y-5">
        <div className="flex items-center gap-4 pb-6 border-b border-slate-800">
          <div className="w-16 h-16 rounded-full bg-slate-800 border-2 border-emerald-500 flex items-center justify-center text-emerald-400 font-extrabold text-2xl">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-300 mb-1">Farm Size</label>
            <input
              type="text"
        ...
        <div className="pt-2 flex justify-end">
          <button
            type="submit"
            className="px-6 py-2.5 text-sm font-bold text-white bg-emerald-600 hover:bg-emerald-500 rounded-xl shadow-lg shadow-emerald-900/30 transition-all"
          >
            Save Changes
          </button>
        </div>
